...
class OREOLogCallback(LogCallback):
    @override
    def on_log(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        if not args.should_save:
            return
        ...
        logs = dict(
            current_steps=self.cur_steps,
            total_steps=self.max_steps,
            loss_reward=state.log_history[-1].get("loss_reward"),
            loss_policy=state.log_history[-1].get("loss_policy"),
            kl_estimate=state.log_history[-1].get("kl_estimate"),
            eval_loss=state.log_history[-1].get("eval_loss"),
            predict_loss=state.log_history[-1].get("predict_loss"),
            reward=state.log_history[-1].get("reward"),
            accuracy=state.log_history[-1].get("rewards/accuracies"),
            policy_lr=state.log_history[-1].get("policy_learning_rate"),
            reward_lr=state.log_history[-1].get("reward_learning_rate"),
            epoch=state.log_history[-1].get("epoch"),
            percentage=round(self.cur_steps / self.max_steps * 100, 2) if self.max_steps != 0 else 100,
            elapsed_time=self.elapsed_time,
            remaining_time=self.remaining_time,
