...
def convert_alpaca(
    ...
    if dataset_attr.kto_tag:
        if dataset_attr.formatting == "alpaca_custom_kto":
            # format of the kto_tag is column#true=<label_for_true_val0,label_for_true_val1,...>/false=<label_for_false_val0,...>
            kto_col, label_pattern = dataset_attr.kto_tag.split("#")
            true_pattern = label_pattern.split("/")[0]
            true_vals = true_pattern.split("=")[1].split(",")
            if isinstance(example[kto_col], str) or isinstance(example[kto_col], int):
                response = [{"role": Role.ASSISTANT.value, "content": example[dataset_attr.response]}]
                if example[kto_col] in true_vals:
                    response = response + [{"role": Role.ASSISTANT.value, "content": ""}]
                else:
                    response = [{"role": Role.ASSISTANT.value, "content": ""}] + response
        else:
            if isinstance(example[dataset_attr.kto_tag], bool):  # kto example
                response = [{"role": Role.ASSISTANT.value, "content": example[dataset_attr.response]}]
